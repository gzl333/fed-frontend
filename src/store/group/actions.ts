...
const actions: ActionTree<GroupModuleInterface, StateInterface> = {
  ...
  /* 新建group */
  async switchMemberRole (context, payload: { group_id: string, member_id: string, role: string }) {
    // 切换逻辑
    payload.role = payload.role === 'leader' ? 'member' : 'leader'
    // 发送请求
    const respEditGroupRole = await context.dispatch('editGroupRole', payload)
    // 更新member信息
    const memberObj = respEditGroupRole.data
    context.commit('updateSingleMember', {
      groupId: payload.group_id,
      memberObj
    })
  },
  async editGroupRole (context, payload: { member_id: string; role: string }) {
    const api = apiBase + '/vo/members/' + payload.member_id + '/role/' + payload.role + '/'
    const response = await axios.post(api)
    return response
  },
  async removeGroupMember (context, payload: { groupId: string; username: string }) {
    const usernames: string[] = []
    usernames.push(payload.username)
    await context.dispatch('delGroupMember', { groupId: payload.groupId, usernames: usernames })
    await context.dispatch('loadGroupMemberTableFromGroup')
  },
  async delGroupMember (context, payload: { groupId: string, usernames: string[]}) {
    const api = apiBase + '/vo/' + payload.groupId + '/remove-members/'
    const response = await axios.post(api, { usernames: payload.usernames })
    return response
  },
  async insertGroupMember (context, payload: { groupId: string, username: string }) {
    const usernames: string[] = []
    usernames.push(payload.username)
    await context.dispatch('addGroupMember', { groupId: payload.groupId, usernames: usernames })
  },
  async addGroupMember (context, payload: { groupId: { value: string }, usernames: string[]}) {
    const api = apiBase + '/vo/' + payload.groupId.value + '/add-members/'
    const response = await axios.post(api, { usernames: payload.usernames })
    const memberObj = response.data.success
    context.commit('updateMember', {
      groupId: payload.groupId.value,
      memberObj
    })
