...
const actions: ActionTree<VmModuleInterface, StateInterface> = {
  ...
  updateVmTable (/* context */) {
    ...
    //     //     // }
    //     //     // if (!context.state.tables.globalImageTable.isLoaded) {
  ...
  //     }
  //     // 发送请求
  //     const server = payload.isGroup ? context.state.tables.groupServerTable.byId[payload.serverId] : context.state.tables.providerServerTable.byId[payload.serverId]
  //     // 去掉协议
  //     const endpoint_url = server.endpoint_url.substr(server.endpoint_url.indexOf('//'))
  //     // 判断结尾有没有'/'，并加上当前用户使用的协议
