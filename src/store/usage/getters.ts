...
const getters: GetterTree<UsageInterface, StateInterface> = {
  ...
  getServiceOptions (state): { value: string; label: string; }[] {
    ...
    */
    ...
    for (const service of Object.values(state.availableServiceTable.byId)) {
      serviceOptions.push(
        {
          value: service.id,
  ...
  getPublicNetworksByServiceId (state, serviceId: string): NetworkInterface[] {
    return Object.values(state.availableNetworkTable.byLocalId).filter(network => network.public && network.service === serviceId)
  },
  ...
  getImagesByServiceId (state, serviceId: string): NetworkInterface[] {
    return Object.values(state.availableNetworkTable.byLocalId).filter(network => network.service === serviceId)
  },
